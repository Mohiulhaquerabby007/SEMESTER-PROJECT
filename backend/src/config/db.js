...
const connectDB = async () => {
  try {
    ...
    // Auto-seed if the database is empty (in-memory or fresh Atlas)
...
/** Populate test data only when the DB has no users at all */
const autoSeedIfEmpty = async () => {
  try {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const price = 60 + distance * 15 + { document: 0, small: 10, medium: 25, large: 50 }[parcelType];
    ...
    console.log(`✅ Auto-seed complete: ${users.length} users, ${riders.length} riders, ${rawOrders.length} orders`);
